...
def train_one_epoch(model:nn.Module, trainDataSet, criterion, optimizer, scheduler):
    ...
    # for index, (images, labels) in tqdm(enumerate(trainDataSet)):
        ...
        lossList.append(loss.data[0])
        if index % 200 == 0:
            print("index:", index, "loss:", loss.data[0])
    scheduler.step()
...
def get_test_data_set_accuracy(model,imageSize,batchSize):
    ...
    for index, (images, labels) in enumerate(testDataSet):
        ...
        rightCount += np.sum(predictLabel == labels.data)
        totalCount += labels.shape[0]
    return round(rightCount/totalCount,4)
    ...
    for epoch in tqdm(range(epochs),desc="Training"):
        ...
        if validateAccuracy > currentBestAccuracy:
            ...
            if noProgressEpochs>=maxBearableEpochs:
                ...
                break
        stopEpoch=epoch
    testAccuracy=get_test_data_set_accuracy(model,imageSize,batchSize)
    print("==========================================================================================")
    print("testAccuracy",testAccuracy,"validateAccuracy",currentBestAccuracy,"bestEpoch",currentBestEpoch,"stopEpoch",stopEpoch)
    print("==========================================================================================")
...
if __name__=="__main__":
    ...
    # train(model=vitModel,modelName="ViT",learningRate=5e-5,epochs=200,etaMin=1e-5,imageSize=224,batchSize=64,savedName="2.pkl",)
